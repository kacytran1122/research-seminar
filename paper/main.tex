...
This paper examines that question using real paired Sentinel-2 image chips and ICESat-2 photon-derived features. The empirical goal is to compare lightweight, reproducible fusion operators under a shared training and evaluation protocol. The study is designed around venue-grade reporting: results are tied to saved training artifacts, exported figures are color and publication-oriented, and reproducibility metadata is retained with the manuscript package.

The contributions are threefold: a paired optical--photon fusion pipeline for 128$\times$128 Sentinel-2 chips and ICESat-2 tabular features; a comparison of five fusion operators using real completed A6000 runs; and a reproducibility package containing metric tables, figure sources, and provenance metadata.
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        font=\scriptsize,
        box/.style={draw=#1, rounded corners=3pt, line width=0.8pt, fill=#1!6,
                    minimum width=2.15cm, minimum height=0.70cm, align=center},
        arrow/.style={-{Stealth[length=7pt,width=8pt]}, line width=1.0pt, draw=black}
    ]
    \node[box=blue] (s2) at (0,1.0) {\textbf{Sentinel-2 RGB}\\128 x 128 chips};
    \node[box=green!70!black] (is2) at (0,-1.0) {\textbf{ICESat-2 photons}\\10 tabular features};
    \node[box=cyan!80!black] (img) at (3.1,1.0) {\textbf{Image branch}\\spatial encoder};
    \node[box=green!70!black] (pho) at (3.1,-1.0) {\textbf{Photon branch}\\MLP + dropout};
    \node[box=orange!90!black, minimum width=2.45cm] (fus) at (6.25,0) {\textbf{Fusion operator}\\concat / product\\average / gate};
    \node[box=purple, minimum width=2.15cm] (clf) at (9.2,0) {\textbf{Classifier head}\\3-class logits};
    \node[box=red!75!black, minimum width=1.65cm] (out) at (11.75,0) {\textbf{Output}\\class map};
    \draw[arrow] (s2.east) -- (img.west);
    \draw[arrow] (is2.east) -- (pho.west);
    \draw[arrow] (img.east) -- (fus.west);
    \draw[arrow] (pho.east) -- (fus.west);
    \draw[arrow] (fus.east) -- (clf.west);
    \draw[arrow] (clf.east) -- (out.west);
    \node[align=center, font=\tiny] at (6.25,-1.45)
        {Held-out evaluation: accuracy, macro-F1, balanced accuracy, kappa, calibration, confusion matrix};
    \end{tikzpicture}}
    \caption{Color schematic of the paired Sentinel-2/ICESat-2 fusion pipeline with explicit directional arrows.}
    \label{fig:pipeline}
\end{figure}
...
\midrule
Fixed avg. & 0.9567 & 0.7408 & 0.7143 & 0.7272 & 0.0344 \\
Concat & 0.9464 & 0.6910 & 0.6842 & 0.0492 & 0.5620 \\
Avail.-aware gate & 0.9437 & 0.6523 & 0.6399 & 0.6513 & 0.0519 \\
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.90\linewidth]{figures/fig_results_scores_bw.pdf}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.64\linewidth]{figures/fig_product_confusion_bw.pdf}
    \caption{Row-normalized product-fusion confusion matrix with raw counts.}
    \label{fig:cm_product}
\end{figure}

\section{Discussion}
The results suggest that fixed averaging can be a strong regularized baseline for this held-out split, while product and gated fusion may be more sensitive to class imbalance or modality-specific noise. The concat result also deserves follow-up audit because its kappa is much lower than expected from its high accuracy and macro-F1; the manuscript reports this inconsistency explicitly rather than hiding it.

\section{Conclusion}
This paper presents a reproducible paired Sentinel-2/ICESat-2 multimodal classification study designed for IEEE TGRS-style reporting. Five fusion operators were evaluated using real completed A6000 outputs. Fixed-average fusion gives the strongest held-out result, with 0.9567 accuracy and 0.7408 macro-F1. The main takeaway is that macro-F1, balanced accuracy, kappa, calibration, and confusion matrices should accompany accuracy in remote-sensing multimodal evaluation.
